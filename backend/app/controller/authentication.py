...
async def login(request_body: LoginSchema):
    ...
    user = await AuthService.get_user_by_credentials(request_body.username, request_body.password, request_body.role)
    return ResponseSchema(detail="Successfully login", result={"token_type": "Bearer", "access_token": token, "username": user.username, "email": user.email, "role": user.role})
